...
public class ProtoRelConverter {
  ...
  private Filter newFilter(FilterRel filterRel) {
    ...
    ProtoExpressionConverter expressionConverter =
  ...
  private NamedStruct newNamedStruct(ReadRel readRel) {
    ...
    // FIXME: put in util
    boolean isNullable =
        protoStruct.getNullability() == io.substrait.proto.Type.Nullability.NULLABILITY_NULLABLE;
    Type.Struct struct = Type.Struct.builder().fields(types).nullable(isNullable).build();
    return ImmutableNamedStruct.builder()
        .names(protoNamedStruct.getNamesList())
        .struct(struct)
        .build();
  }
  ...
  private EmptyScan newEmptyScan(ReadRel readRel) {
    ...
    if (readRel.hasFilter()) {
      ProtoExpressionConverter expressionConverter =
          new ProtoExpressionConverter(lookup, extensions, rootType);
  ...
  private NamedScan newNamedScan(ReadRel readRel) {
    ...
    ProtoExpressionConverter expressionConverter =
        new ProtoExpressionConverter(lookup, extensions, rootType);
    ImmutableNamedScan.Builder builder = NamedScan.builder().initialSchema(namedStruct);
  ...
  private VirtualTableScan newVirtualTable(ReadRel readRel) {
    ...
    for (io.substrait.proto.Expression.Literal.Struct struct : virtualTable.getValuesList()) {
      ...
      ImmutableExpression.StructLiteral structLiteral =
          ImmutableExpression.StructLiteral.builder().fields(literals).build();
  ...
  private Fetch newFetch(FetchRel fetchRel) {
    ...
    return builder.input(input).count(fetchRel.getCount()).offset(fetchRel.getOffset()).build();
  }
  ...
  private Aggregate newAggregate(AggregateRel aggregateRel) {
    ...
    for (AggregateRel.Grouping protoGrouping : aggregateRel.getGroupingsList()) {
      ...
      for (io.substrait.proto.Expression protoExpression :
          protoGrouping.getGroupingExpressionsList()) {
        Expression expression = expressionConverter.from(protoExpression);
        expressions.add(expression);
      }
    ...
    for (AggregateRel.Measure protoMeasure : aggregateRel.getMeasuresList()) {
      ...
      SimpleExtension.AggregateFunctionVariant declaration =
          lookup.getAggregateFunction(funcRef, extensions);
  ...
  private Join newJoin(JoinRel joinRel) {
    ...
    var builder = Join.builder().condition(condition).joinType(joinType).left(left).right(right);
